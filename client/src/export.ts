...
export async function exportPdf(
  ...
  // Fetch original PDF
  const res = await fetch(`/api/pdf?path=${encodeURIComponent(pdfPath)}&token=${encodeURIComponent(token)}`);
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    outPage.drawImage(pngImage, { x: 0, y: 0, width: outPage.getWidth(), height: outPage.getHeight() });
  }
