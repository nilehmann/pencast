...
<div class="browser">
  <div class="toolbar">
    <button onclick={goBack} disabled={pathHistory.length === 0}>← Back</button>
      ...
      {#each entries as entry (entry.path)}
        <li>
          {#if entry.type === 'folder'}
            <button class="entry folder" onclick={() => navigate(entry)}>
              📁 {entry.name}
            </button>
          {:else}
            <button class="entry pdf" onclick={() => loadPdf(entry)}>
              📄 {entry.name}
            </button>
          {/if}
        </li>
